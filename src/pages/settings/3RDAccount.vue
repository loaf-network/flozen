...
async function openQrLogin() {
  ...
  try {
    ...
    if (keyRes.code !== 200 || !keyRes.data?.unikey) { qrStatus.value = "error"; qrText.value = "获取失败"; return }
    const key = keyRes.data.unikey
    const createRes = await createQr(key)
    if (createRes.code !== 200 || !createRes.data?.qrimg) { qrStatus.value = "error"; qrText.value = "生成失败"; return }
    ...
    polling = setInterval(async () => {
      try {
        ...
        if (check.code === 803 && check.cookie) {
          cleanup(); ncmCookie.value = check.cookie
          await saveConfig("ncmCookie", check.cookie)
          await fetchUser(check.cookie)
          showQr.value = false
        } else if (check.code === 800) {
          cleanup(); qrStatus.value = "expired"; qrText.value = "二维码已过期"
        } else if (check.code === 802) {
          qrText.value = "已扫码，请在手机上确认"
        }
      } catch { /* ignore */ }
    }, 2000)
  } catch { qrStatus.value = "error"; qrText.value = "网络错误" }
}

async function logout() {
  user.value = null; ncmCookie.value = ""
  await saveConfig("ncmCookie", ""); await saveConfig("ncmUser", null)
}

function cleanup() { if (polling) { clearInterval(polling); polling = null } }
</script>
  ...
  <div class="p-6">
    ...
    <Card class="py-0 gap-0 overflow-hidden">
      <div v-if="user" class="flex items-center gap-3 px-4 py-3">
        ...
        <Button variant="ghost" size="sm" class="text-destructive" @click="logout">退出登录</Button>
      </div>
    ...
    <Dialog v-model:open="showQr" @update:open="(v) => { if (!v) cleanup() }">
      <DialogContent class="sm:max-w-xs">
        ...
        <div class="flex flex-col items-center gap-3 py-2">
          <div v-if="qrStatus === 'loading'" class="size-44 rounded-xl border border-border flex items-center justify-center">
            <Loader2 :size="24" class="animate-spin text-muted-foreground" />
          </div>
          <img v-else-if="qrImg" :src="qrImg" class="size-44 rounded-xl" />
          <div v-else class="size-44 rounded-xl border border-dashed border-border flex items-center justify-center">
            <QrCode :size="28" :stroke-width="1" class="text-muted-foreground" />
          </div>
          <p class="text-xs text-muted-foreground">{{ qrText }}</p>
          <Button v-if="qrStatus === 'expired' || qrStatus === 'error'" variant="outline" size="sm" @click="openQrLogin">重新生成</Button>
        </div>
      </DialogContent>
    </Dialog>
  </div>
